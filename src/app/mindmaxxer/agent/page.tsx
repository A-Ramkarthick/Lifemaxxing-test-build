...
export default function MindMaxxerAgentPage() {
  return (
    <NokiaLayout>
      <div className="flex flex-col h-full space-y-4 pt-2">
        <div className="flex items-center gap-2 border-b-2 border-nokia-dim/30 pb-2 mb-2">
          <Link href="/mindmaxxer">
            <button className="p-1 hover:bg-nokia-dim/20 rounded-sm">
              <ArrowLeft className="w-5 h-5" />
            </button>
          </Link>
          <h1 className="text-xl font-bold uppercase tracking-wider">Mind Link</h1>
        </div>

        <AgentVideoFeed agentName="MindMaxxer" />

        <div className="flex-1 min-h-0 border-t-2 border-nokia-dim/30 pt-2">
           <AgentChat 
             agentName="MindMaxxer" 
             initialMessage="I've processed your latest journal entries. Do you want to discuss the pattern of anxiety detected in the evenings?"
           />
        </div>
      </div>
    </NokiaLayout>
