    ...
    <NokiaLayout>
      <div className="flex flex-col h-full space-y-4 pt-2">
        <div className="flex items-center gap-2 border-b-2 border-nokia-dim/30 pb-2 mb-2">
          <Link href="/careermaxxer">
            <button className="p-1 hover:bg-nokia-dim/20 rounded-sm">
              <ArrowLeft className="w-5 h-5" />
            </button>
          </Link>
          <h1 className="text-xl font-bold uppercase tracking-wider">Career Comms</h1>
        </div>

        <AgentVideoFeed agentName="CareerMaxxer" />

        <div className="flex-1 min-h-0 border-t-2 border-nokia-dim/30 pt-2">
           <AgentChat 
             agentName="CareerMaxxer" 
             initialMessage="I've analyzed the job market trends. We need to optimize your GitHub profile for AI engineering roles. Ready to strategize?"
           />
        </div>
      </div>
    </NokiaLayout>
